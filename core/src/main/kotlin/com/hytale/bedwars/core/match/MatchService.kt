...
class MatchService(private val stateMachine: MatchStateMachine = MatchStateMachine()) {
    fun createMatch(
        ...
        val teams = teamTemplates.mapIndexed { index, template ->
            Team(
        ...
        val sessions = players.associateWith { playerId ->
    ...
    fun transition(match: Match, next: MatchState) {
    ...
    fun assignTeams(teams: MutableList<Team>, sessions: MutableMap<UUID, PlayerSession>) {
    ...
    fun markEliminatedIfNeeded(team: Team, sessions: Map<UUID, PlayerSession>) {
        if (team.alivePlayers(sessions).isEmpty()) {
            team.state = TeamState.ELIMINATED
        }
    }

    fun registerGenerators(match: Match, generators: List<Generator>) {
